...
handler.before = async function (m, { user, isBotAdmin, isAdmin }) {
  ...
  if (chat.antiLink && isGroupLink) {
    await m.reply(`*「 ANTI LINKS 」*\n*Hasta la vista puta👋, ${await this.getName(m.sender)} rompiste las reglas serás exterminado....!!*`)
    await m.reply(`*Tienes 3 segundos para eliminar el link y retractarte...!!!!*`)
    await m.reply(`*3!!*`)
    await m.reply(`*2!!*`)
    await m.reply(`*1!!*`)
    if (isAdmin) return m.reply('*Te salvaste puta eres admin, no puedo eliminarte :v*')
